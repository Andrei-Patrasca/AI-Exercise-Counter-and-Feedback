...
class RegisterScreen(ctk.CTkFrame):
    def __init__(self, master, on_register_success,
                 on_go_to_login, on_go_to_verify):
    ...
    def _build(self):
        ...
        card = ctk.CTkFrame(self, fg_color=COLORS["surface"],
                             corner_radius=16)
        ...
        ctk.CTkLabel(card, text="💪 FitTrack",
                     font=FONT_TITLE,
                     text_color=COLORS["accent"]).pack(pady=(40, 4))
        ...
        self.match_lbl = ctk.CTkLabel(card, text="", font=FONT_SMALL,
                                       text_color=COLORS["accent"])
        self.match_lbl.pack(pady=(2, 0))

        self.error_lbl = ctk.CTkLabel(card, text="", font=FONT_SMALL,
                                       text_color=COLORS["error"])
        self.error_lbl.pack(pady=(2, 0))

        self.reg_btn = ctk.CTkButton(
            card, text="Create Account", width=320, height=44,
            font=FONT_H3, command=self._do_register,
    ...
    def _check_match(self):
        ...
        if p1 == p2:
            self.match_lbl.configure(
                text="✓ Passwords match", text_color=COLORS["accent"])
        else:
            self.match_lbl.configure(
    ...
    def _do_register(self):
        ...
        self.on_go_to_verify(self._pending)
