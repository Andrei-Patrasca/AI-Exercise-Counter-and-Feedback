...
class WorkoutScreen(ctk.CTkFrame):
    ...
    def _build_left(self):
        ...
        ctk.CTkLabel(left, text="Controls",
                     font=FONT_H3,
                     text_color=COLORS["accent"]).pack(pady=(24, 16))
        ...
        # Finish button (hidden until started)
    ...
    def _build_right(self):
        ...
        self.log_frame = ctk.CTkScrollableFrame(
            right, fg_color=COLORS["surface_alt"],
            corner_radius=8
        )
        ...
        self.lbl_pushups= ctk.CTkLabel(totals, text="Push-ups:     0",
                                        font=FONT_BODY,
                                        text_color=COLORS["text"])
        self.lbl_curls.pack(anchor="w", pady=2)
        self.lbl_squats.pack(anchor="w", pady=2)
        self.lbl_pushups.pack(anchor="w", pady=2)
    ...
    def _flush_reps(self):
        for exercise in ["bicep_curl", "squat", "pushup"]:
            current = self.counter.get_count(exercise)
            saved   = self.last_rep_counts[exercise]
            diff    = current - saved
            if diff > 0:
                self.workout_manager.log_set(exercise, diff)
                self.last_rep_counts[exercise] = current
    ...
    def _update_frame(self):
        ...
        if exercise != "unknown" and exercise != self.last_exercise:
            ...
            self.exercise_lbl.configure(
                text=names.get(exercise, "Detecting...")
            )
    ...
    def _on_rep(self, exercise):
        ...
        name = names.get(exercise, exercise)

        # Add to front of log list
        self.log_entries.insert(0, f"• {total} × {name}")

        # Clear and redraw log
