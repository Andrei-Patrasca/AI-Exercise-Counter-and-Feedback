...
class VerifyEmailScreen(ctk.CTkFrame):
    def __init__(self, master, email, code,
        ...
        self.email       = email
        self.code        = code          # the code we sent
        self.on_verified = on_verified   # called when code matches
        self.on_back     = on_back       # back to register
        self._build()
    ...
    def _build(self):
        ...
        ctk.CTkLabel(
            card,
            text=f"We sent a 6-digit code to:\n{self.email}",
            font=FONT_BODY,
