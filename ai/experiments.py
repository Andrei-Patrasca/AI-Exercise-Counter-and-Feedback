...
Reads the same CSVs your normal training uses (ai/training_data/*.csv).
Writes everything to a new folder  ai/experiments/  so nothing else is overwritten:

    ai/experiments/split_comparison.png
    ai/experiments/split_comparison.csv
    ai/experiments/grid_search_results.csv
    ai/experiments/grid_search_top.csv

Place this file in the ai/ folder and run from the project root:
    python ai/experiments.py

Nothing here saves a .keras model, so your exercise_model.keras stays exactly as it is.
"""
import os
import glob
import itertools
...
def experiment_grid(X, y, n_classes):
    ...
    # Hyperparameter grid (kept modest so it finishes in a reasonable time)
