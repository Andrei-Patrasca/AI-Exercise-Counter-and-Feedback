...
Generates four plots for the thesis from your REAL trained model and data.
Run AFTER train_classifier.py has produced exercise_model.keras.

Place in the ai/ folder, run from project root:
    python ai/generate_thesis_plots.py

Produces (in ai/):
    dataset_distribution.png   - how many frames per exercise
    training_curves.png        - accuracy + loss over epochs (needs history, see bottom)
    confusion_matrix.png       - 3x3 confusion matrix on test set
    per_class_metrics.png      - precision / recall / F1 per exercise

Also prints overall accuracy and a full classification report to the terminal.
"""
import os
import glob
import numpy as np
...
# -----------------------------------------------------------------------------
# TO ENABLE TRAINING CURVES:
# In train_classifier.py, after the line  history = model.fit(...)  add:
#
#     import numpy as np, os
#     np.save(os.path.join(os.path.dirname(__file__), "training_history.npy"),
#             history.history)
#
# Then re-run train_classifier.py once, and this script will plot the curves.
# -----------------------------------------------------------------------------
