...
class App(ctk.CTk):
    ...
    def _show_verify(self, pending):
        ...
        def on_verified():
            ...
            print(f"Registration result: ok={ok}, msg={msg}")  # debug
            if ok:
                self._show_login()
            else:
                self._show_verify_error(msg, pending)

        self._switch(VerifyEmailScreen(
            self,
            email=pending["email"],
            code=pending["code"],
    ...
    def _show_verify_error(self, msg, pending):
        """Called if registration fails after verification."""
        from ui.verify_email_screen import VerifyEmailScreen
        ...
        self._switch(screen)
        screen.error_lbl.configure(
            text=f"Registration failed: {msg}"
        )
    ...
    def _show_workout(self):
        ...
        self._switch(WorkoutScreen(
            self,
            user=self.current_user,
            on_finish=self._show_dashboard
        ))

    def _show_history(self):
        from ui.history_screen import HistoryScreen
        self._switch(HistoryScreen(
            self,
            user=self.current_user,
